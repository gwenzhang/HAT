...
def create_supervised_trainer(model, optimizer, loss_fn,
    ...
    def _update(engine, batch):
        ...
        # attribute = attribute.to(device) if torch.cuda.device_count() >= 1 else attribute
        # attribute = torch.abs(attribute - 1)
        # attribute = torch.cat((attribute,attribute_filp), dim=1)
...
def do_train(
    ...
    def log_validation_results(engine):
        # if engine.state.epoch % eval_period == 0 or ((engine.state.epoch>140) and (engine.state.epoch % 2 == 0)) :
